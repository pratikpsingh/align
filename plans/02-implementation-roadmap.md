...
- [ ] Train one target-conditioned actor on the selected formation templates and ground-to-formation task.
  ...
  - [x] Implement a matched replay of the saved takeoff commands with four active drones, one isolated active drone, and a no-downwash simulator intervention; CPU extraction/audit and three physical arms passed in `20260919T011921.157537IST-01376539`. The four-drone arm reproduced contact, while isolated and no-downwash arms stayed airborne; modeled downwash is supported for this fixed-command simulator fall. A safe wake-avoidance control intervention and frozen-policy evaluation remain open. See [the replay contract](../docs/44-takeoff-controller-replay.md).
  - [x] Implement a bounded local wake-clearance command candidate and exact-command host audit; CPU checks pass. One-arm physical trace and immutable CPU analysis passed (`20260919T013621.050067IST-5e728518`, `20260919T013845.620755IST-45631a55`): no contact, 0.717 m minimum takeoff separation, 82 interventions. Closed-loop frozen-policy evaluation and a same-image raw comparator passed on the GPU (baseline `20260919T015013.905042IST-20bb0cd3`, guard `20260919T015207.324862IST-48221710`, comparison `20260919T015609.579036IST-5a799800`). The guard reduced contacts 8→4 and permitted 1,102 formation environment rows versus zero, but neither arm recorded a success; later formation falls remain unresolved. See [the guard contract](../docs/45-local-wake-clearance.md) and [the frozen-policy replay](../docs/46-frozen-policy-wake-guard.md).
...
- [ ] Evaluate the frozen actor on multiple swarm sizes; record infeasible geometry, memory limits, and failures explicitly.
  - [x] Establish a physical eight-drone feasibility reference under exact-target control. Run 20260919T005232.351407IST-3c858751 passed two repeated episodes, each successful at 14.59 s with 0.66305 m minimum post-ground separation and zero guarded airborne contact. This is not learned-policy size generalization.
