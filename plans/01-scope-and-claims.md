...
IDs identify requirements and experiments; they do not imply implementation order. The register states final capability-level acceptance requirements; supporting geometry, task, reward, observation, vector-environment, rollout, and recurrent-network contracts are implemented, while collection, PPO, recovery, and learned-policy evidence remain pending.
