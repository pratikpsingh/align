"""Launch a deterministic four-drone fixed-ID waypoint flight."""
...
def waypoint_command(
    ...
    return [
        *docker,
        ...
        "--network=none",
        "--name",
        f"align-waypoint-route-{run.name.lower()}",
        "--runtime=nvidia",
        "--gpus",
        f"device={gpu}",
        "-e",
...
def run_main(argv: list[str] | None = None) -> int:
    ...
    parser.add_argument("--task-config", type=Path, default=Path("configs/waypoint-task-48s.json"))
    parser.add_argument(
    ...
    parser.add_argument(
        "--route-config",
        type=Path,
        default=Path("configs/waypoint-route-3m.json"),
    )
    ...
    route = WaypointRouteConfig.from_dict(values["route"])
    ...
    if construction.num_agents != 4 or task.num_envs != 4:
        raise ValueError("first physical waypoint acceptance requires four drones/worlds")
    ...
    run = create_run_directory(root / "runs/waypoint-reference")
    ...
    bundle["formation_schedule"] = FormationScheduleConfig(
        kinds=(construction.formation_kind,), seed=construction.seed
    ).to_dict()
    write_json_atomic(run / "config.json", bundle)
    write_json_atomic(run / "waypoint-route-config.json", route.to_dict())
    ...
    report.update(
        ...
        waypoint_route_config_sha256=digest(run / "waypoint-route-config.json"),
        plan=plan.to_dict(),
    ...
    print(f"{report['started_at_ist']} IST Waypoint reference: {run}", flush=True)
    ...
    try:
        ...
        if (
            ...
            or metrics.get("waypoint_route_commands", 0) < task.num_envs
        ):
            raise RuntimeError("physical waypoint reference probe failed")
        report["audit"] = audit_waypoint_route(
            run / "evaluation" / "evaluation.csv",
            run / "evaluation" / "policy-telemetry.csv",
            run / "evaluation" / "waypoint-progress.csv",
            plan,
            route,
            control_dt_seconds=construction.physics_dt * construction.control_decimation,
            formation_kind=construction.formation_kind,
        )
        ...
        if report["status"] != "passed":
            report["container_cleanup"] = capture(
                [*docker, "rm", "--force", f"align-waypoint-route-{run.name.lower()}"],
                timeout=30,
            )
