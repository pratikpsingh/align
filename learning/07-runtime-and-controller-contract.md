...
The flight itself completed, but PDF export failed because the vendor image's matplotlib lacked fontTools. This distinguishes a physical experiment from a complete usable result bundle. We added that dependency explicitly and preserved the failed attempt. We also corrected root-container JSON permissions so the host can inspect the completed records. The acceptance run uses tighter thresholds frozen from the saved calibration data.
