...
A policy can contact the ground during takeoff and terminate before the formation phase. That is a failed **flight**, but its raw trace can still be a valid telemetry capture. For this narrow case, the host launcher accepts the older simulator evaluation gate failing solely because formation was never entered, provided reason code 3 records contact and every other checkpoint, finite-data, row-count, and reward/geometry check passes. The host report sets capture_mode to early_safety_termination and includes an airborne_contact_audit with the first post-ascent contact for each drone and episode. The independent CPU analysis command below adds the same audit to a hash-linked report. Neither capture status nor a contact count proves mission success.
